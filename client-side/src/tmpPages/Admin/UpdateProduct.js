  ...
  const getSingleProduct = async () => {
    try {
      const { data } = await axios.get(
        `/api/v1/tmp/product/get-product/${params.slug}`
      );
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      const { data } = axios.put(
        `/api/v1/tmp/product/update-product/${id}`,
        productData
      );
      if (data?.success) {
        ...
        navigate("/dashboard/tmp/admin/products");
      }
    } catch (error) {
      console.log(error);
  ...
  const handleDelete = async () => {
    try {
      ...
      const { data } = await axios.delete(
        `/api/v1/tmp/product/delete-product/${id}`
      );
            ...
            <div className="">
              ...
              <div className="mb-3">
                {photo ? (
                  ...
                  <div className="text-center">
                    <img
                      src={`/api/v1/tmp/product/product-photo/${id}`}
                      alt="product_photo"
                      height={"200px"}
                      className="img img-responsive"
                    />
