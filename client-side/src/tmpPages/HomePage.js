...
const HomePage = (userDetails) => {
  ...
  const getAllProducts = async () => {
    try {
      setLoading(true);
      const { data } = await axios.get(`/api/v1/tmp/product/product-list/${page}`);
      setLoading(false);
  ...
  const getTotal = async () => {
    try {
      const { data } = await axios.get("/api/v1/tmp/product/product-count");
      setTotal(data?.total);
    } catch (error) {
      console.log(error);
    }
  ...
  const filterProduct = async () => {
    try {
      const { data } = await axios.post("/api/v1/tmp/product/product-filters", {
        checked,
        radio,
      });
      setProducts(data?.products);
