...
const CartPage = () => {
  ...
  const removeCartItem = (pid) => {
    try {
      ...
      setCart(myCart);
      localStorage.setItem("tmp-cart", JSON.stringify(myCart));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const getToken = async () => {
    try {
      const { data } = await axios.get("/api/v1/tmp/product/braintree/token");
  ...
  const handlePayment = async () => {
    try {
      ...
      const { data } = await axios.post("/api/v1/tmp/product/braintree/payment", {
        nonce,
        cart,
      });
      setLoading(false);
      localStorage.removeItem("tmp-cart");
      setCart([]);
